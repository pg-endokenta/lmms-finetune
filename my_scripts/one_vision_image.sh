...
TRAIN_DATA_PATH=./my_data/ok_vqa/dataset.json  # path to the training data json file
EVAL_DATA_PATH=    # path to the evaluation data json file (optional)
IMAGE_FOLDER=./my_data/ok_vqa/images                      # path to the image root folder; if provided, the image paths in the json should be relative
...
LORA_R=16                                                # the lora rank (both llm and vision encoder)
LORA_ALPHA=32                                            # the lora alpha (both llm and vision encoder)

RUN_ID=${MODEL_ID}_lora-${USE_LORA}_qlora-${Q_LORA}_0615     # a custom run id that determines the checkpoint folder and wandb run name
...
MODEL_MAX_LEN=7500                                       # maximum input length of the model


# 下のコマンドは --eval_data_path $EVAL_DATA_PATH \ を省いている

uv run torchrun $DISTRIBUTED_ARGS train.py \
    --model_id $MODEL_ID \
